...
## 2 调用SpringApplication 的构造方法，进行某些初始参数的设置

当程序开始执行之后，会调用 SpringApplication 的构造方法，进行某些初始参数的设置
...
## 3 SpringBoot 启动的运行方法

SpringBoot 启动的运行方法（run方法），可以看到主要是各种运行环境的准备工作
...
public ConfigurableApplicationContext run(String... args) {
    ...
    // 3、设置系统属性“java.awt.headless”的值，默认为true，用于运行headless服务器，进行简单的图像处理，多用于在缺少显示屏、键盘或者鼠标时的系统配置，很多监控工具如jconsole需要将该值设置为true
