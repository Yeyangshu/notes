...
### 		2.1 通过bean的id获取IOC容器中的对象（上面已经用过）
...
### 		2.4 通过命名空间为bean赋值，简化配置文件中属性声明的写法
...
- byName：按照名字进行装配，以属性名（set方法后面名称首字母小写）作为id去容器中查找组件，进行赋值，如果找不到则装配null
