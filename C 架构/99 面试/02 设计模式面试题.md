...
如模板模式，命令模式，策略模式，适配器模式、桥接模式、装饰模式，观察者模式，状态模式，访问者模式。

### 原型模式，深拷贝和浅拷贝？

浅拷贝，引用类型指针指向内存，共用同一区域
...
模板方法模式：文件解析定义了解析方法，子类实现

策略模式：文件解析器接口，不同文件有不同的解析器

适配器模式：文件搬运使用OSS，适配器不需要指定oss地址，只需要传入对应的文件名即可
